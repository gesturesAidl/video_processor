...
'''
This code had been extracted from GLUON documentation: 
https://cv.gluon.ai/build/examples_action_recognition/feat_custom.html
Link to original code: 
https://cv.gluon.ai/_downloads/e55fae8b6d174808015aab2ef6957d8b/feat_extract.py 

EXTRACT VIDEO FEATURES FROM A PRETRAINED I3D MODEL
'''
...
def read_data(opt, video_name, transform, video_utils):
    ...
    if opt.video_loader:
        if opt.slowfast:
            clip_input = video_utils._video_TSN_decord_slowfast_loader(video_name, decord_vr, duration, segment_indices,
                                                                       skip_offsets)
        else:
...
def main(logger):
    ...
    # bar = IncrementalBar('Completed', max = len(data_list),  suffix='%(index)d/%(max)d - %(percent).1f%% - ETA: %(eta)ds ')
